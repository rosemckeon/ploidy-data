...
benefit_name <- "inbreeding"
benefit_path <- "benefits/inbreeding_cost/"
...
for(benefit_val in benefit_range){
  ...
  for(dist_lvl in dist_range){
    ...
    sim <- sim$data$adults %>% select(gen, ploidy) %>%
      ...
      mutate(
        inbreeding_cost = as.numeric(benefit_val),
        dist_lvl = as.numeric(dist_lvl)
      )
     # tally by grouping variable and keep in the new columns
    counts <- sim %>%
      group_by(gen, ploidy, inbreeding_cost, dist_lvl) %>%
      tally()
    # clear memory
    sim <- NULL; gc(F)
    # combine dataframes
...
all_counts$inbreeding_cost <- all_counts$inbreeding_cost %>%
  factor(levels(as.factor(all_counts$inbreeding_cost))[3:1])
